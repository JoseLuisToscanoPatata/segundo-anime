<template>
    <div
        class="p-6 flex flex-col justify-start items-start bg-pink-100 rounded-lg"
    >
        ...
        <div class="w-full overflow-x-auto overflow-y-auto">
            ...
            >
                ...
                <tbody>
                    <tr
                        v-for="(dato, indice) in datosOrdenados.slice(
                            primero - 1,
                            ultimo
                        )"
                        :key="indice"
                    ...
                    >
                        <template
                            v-for="(columna, idColumna) in columnas"
                            :key="idColumna"
                        >
                            <template
                                v-for="(valorProp, nomProp) in dato"
                                :key="nomProp"
                            >
                                <td
                                ...
                                >
                                    <template v-if="columna.tipo == 'imagen'">
                                        <a :href="valorProp" target="_blank">
                                            <img
                                                :src="valorProp"
                                                alt="image"
                                                :class="imagenes"
...
export default {
    ...
    computed: {
        ...
        ultimo() {
            if (
                this.paginas + this.paginacion * this.paginas >
                this.datosOrdenados.length
            ) {
    ...
    methods: {
            ...
            this.datosOrdenados = this.datos.filter(function (dato) {
                for (const nombreCampo in dato) {
                    for (var i = 0; i < columnas.length; i++) {
                        if (
                            columnas[i].filtrable &&
                            columnas[i].nombre == nombreCampo
                        ) {
        ...
        compararRol(nombre) {
            for (
                let elemento = 0;
                elemento < this.datosOrdenados.length;
                elemento++
            ) {
